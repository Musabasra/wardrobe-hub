...
if (!authUser) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Upload to the 'avatars' bucket we just created
      ...
      setUser({ ...user, profilePic: publicUrl });
  ...
  const saveProfileChanges = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      alert("Could not sync changes to the archive.");
    }
  };
  return (
  <div className="bg-[#F5F5DC] min-h-screen pt-12 pb-20">
    <div className="max-w-4xl mx-auto px-6">
      ...
      {loading ? (
        ...
        <> {/* Start of Main Content Fragment */}
          ...
          <header className="flex flex-col md:flex-row items-center gap-12 mb-16 border-b border-black/5 pb-16">
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-10 mb-6">
                ...
                <button 
                  onClick={() => setConnectionsModal({ open: true, type: 'Followers' })} 
                  className="flex flex-col items-start hover:opacity-60 transition-opacity"
                >
                ...
                <button 
                  onClick={() => setConnectionsModal({ open: true, type: 'Following' })} 
                  className="flex flex-col items-start hover:opacity-60 transition-opacity"
                >
              ...
              myPieces.map((piece) => (
                <div key={piece.id} className="aspect-[3/4] bg-white border border-black/5 group relative overflow-hidden cursor-pointer">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                     <button onClick={() => navigate('/lab')} className="bg-white text-black px-4 py-2 text-[9px] font-bold uppercase tracking-widest flex items-center gap-2 hover:bg-[#F5F5DC] transition-colors">
      ...
      {/* 5. Modals (Outside loading logic so they don't unmount mid-animation) */}
      <AnimatePresence>
        {isEditOpen && (
            ...
            <motion.div layoutId={`post-${selectedPost}`} className="relative bg-white w-full max-w-5xl h-[80vh] flex flex-col md:flex-row shadow-2xl overflow-hidden">
              ...
              <div className="w-full md:w-96 p-8 flex flex-col bg-white">
                ...
                <div className="flex-grow">
                  <p className="text-sm font-light italic opacity-60 mb-6">Outfit composition curated in the Studio.</p>
